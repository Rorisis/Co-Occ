...
def get_ray_direction_with_intrinsics(h, w, intrins):
    i, j = torch.meshgrid(torch.arange(w, dtype=torch.float32, device=intrins.device), torch.arange(h, dtype=torch.float32, device=intrins.device), indexing='xy')
    directions = []
    for index in range(intrins.shape[0]):
        fx, fy, cx, cy = intrins[index, 0, 0], intrins[index, 1, 1], \
                        intrins[index, 0, 2], intrins[index, 1, 2]
        directions_ = torch.stack([(i - cx) / fx, (j - cy) / fy, -torch.ones_like(i).to(intrins.device)], -1)
...
def get_rays(directions, c2ws):
    rays_o = []
    rays_d = []
    for i in range(directions.shape[0]):
        rays_d_ = directions[i] @ c2ws[i][:3,:3].T  # dot product, equals to: [c2w.dot(dir) for dir in dirs]
                    # Translate camera frame's origin to the world frame. It is the origin of all rays.
        rays_o_ = c2ws[i][:3, 3].expand(rays_d_.shape)
